...
def render_positional_points_against() -> None:
    ...
    if selected_year != "All seasons":
        position_totals = position_totals[position_totals["year"] == selected_year]
        detail = detail[detail["year"] == selected_year]
    ...
    owner_chart = (
        pd.DataFrame({"selected_position": STARTER_POSITION_ORDER})
    ...
    selected_owner_position = summary[
        (summary["owner"] == selected_owner)
        & (summary["selected_position"] == selected_position)
    ]
    selected_league_avg = league_avg[
    ...
    difference = round(owner_avg - league_position_avg, 2)
    ...
    with st.container(border=True):
        st.subheader("Owner vs league average")
        grouped_bar_chart(
            chart_data,
            x="selected_position",
            y="points",
    ...
    with left:
        with st.container(border=True):
            st.subheader(f"{selected_position} leaderboard")
    ...
    with right:
        with st.container(border=True):
            ...
            log = log[
                [
                    "year", "week", "owner", "opponent", "opponent_team_name",
...
def render_roster_matchups() -> None:
    ...
    position_order = ROSTER_POSITION_ORDER
