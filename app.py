...
def render_overview() -> None:
    ...
    standings = run_query(
        """
        SELECT team_owner, wins, losses, playoff_wins, playoff_losses,
               total_wins, total_losses, ties
        FROM standings
        WHERE year = ?
        ORDER BY wins DESC, losses, team_owner
        """,
        (latest_year,),
    )
...
def render_current_season() -> None:
    ...
    current = run_query(
        """
        SELECT team_owner, wins, losses, ties,
               wins || '-' || losses || '-' || ties AS record
        FROM standings WHERE year = ?
        ORDER BY wins DESC, losses, team_owner
        """,
        (latest_year,),
    )
    ...
    season_view = season_view[
        [
            "team_owner", "wins", "losses", "ties", "record", "current_streak",
